...
$SelfScriptName = Split-Path -Leaf $MyInvocation.MyCommand.Path
...
try {
  ...
  Sync-WorkspaceEntries -SourceRoot $WorkspaceSource -DestinationRoot $WorkspaceDestination -SkipNames @($SelfScriptName)
