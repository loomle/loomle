...
TSharedPtr<FJsonObject> MakeVariableReference(
    ...
    const FString& MemberParent = FString())
{
    TSharedPtr<FJsonObject> GuidObject = MakeShared<FJsonObject>();
    Lookup.Field(
        GuidObject,
        TEXT("A"),
        MakeShared<FJsonValueNumber>(static_cast<int32>(Guid.A)));
    Lookup.Field(
        GuidObject,
        TEXT("B"),
        MakeShared<FJsonValueNumber>(static_cast<int32>(Guid.B)));
    Lookup.Field(
        GuidObject,
        TEXT("C"),
        MakeShared<FJsonValueNumber>(static_cast<int32>(Guid.C)));
    Lookup.Field(
        GuidObject,
        TEXT("D"),
        MakeShared<FJsonValueNumber>(static_cast<int32>(Guid.D)));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FSalReferenceIndexExternalOwnerTest,
    "Loomle.Sal.ReferenceIndex.ExternalVariableOwner",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FSalReferenceIndexExternalOwnerTest::RunTest(
    const FString& Parameters)
{
    const FGuid VariableGuid(
        ...
        0x83572468,
        0xa4681357);
    const FString OwnerExportText =
        TEXT("BlueprintGeneratedClass'/Game/BP_Target.BP_Target_C'");

    Loomle::Sal::FReferenceIndexTarget Target;
    Target.Identity.Kind =
        Loomle::Sal::EReferenceDeclarationKind::BlueprintMemberVariable;
    Target.Identity.OwnerPath = TEXT("/Game/BP_Target.BP_Target");
    Target.Identity.Guid = VariableGuid;
    Target.Identity.Name = TEXT("Health");
    Target.OwnerClassPath =
        FTopLevelAssetPath(TEXT("/Game/BP_Target.BP_Target_C"));

    auto ScanOwner = [&Target, &VariableGuid](
        const FString& MemberParent)
    {
        FLookupBuilder Lookup;
        const TSharedPtr<FJsonObject> Node = MakeNode(
            ...
            MakeVariableReference(
                ...
                true,
                MemberParent));
        return Loomle::Sal::FSalReferenceIndex::ScanDecodedForTesting(
            MakeRoot(
                Lookup,
                {MakeShared<FJsonValueObject>(
                    MakeGraph(
                        ...
                        {MakeShared<FJsonValueObject>(Node)}))}),
            Lookup.Values,
            TEXT("/Game/BP_Consumer.BP_Consumer"),
            {},
            Target);
    };

    const Loomle::Sal::FReferenceIndexScanResult Matching =
        ScanOwner(OwnerExportText);
    TestTrue(
        TEXT("Signed GUID words and exact external owner are parsed"),
        Matching.Status
            == Loomle::Sal::EReferenceIndexScanStatus::Parsed);
    TestEqual(
        TEXT("Exact external owner emits one fact"),
        Matching.Sites.Num(),
        1);

    const Loomle::Sal::FReferenceIndexScanResult WrongOwner =
        ScanOwner(
            TEXT("BlueprintGeneratedClass'/Game/BP_Other.BP_Other_C'"));
    TestTrue(
        TEXT("Wrong external owner remains a valid parsed index"),
        WrongOwner.Status
            == Loomle::Sal::EReferenceIndexScanStatus::Parsed);
    TestTrue(
        TEXT("Same GUID under a different owner emits no fact"),
        WrongOwner.Sites.IsEmpty());
    return true;
}
