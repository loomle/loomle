...
Guid and no `state_tree(...)` constructor. Typed ids are scoped inside this
target.
...
The operation-less `query omle` is the exact target read.
...
Property Binding arrows incident to the selected object. Exact reads may append
`with schema`.
...
StateTree `references` searches only the bound StateTree and does not support
`in project`. It reports each factual use-site with exact member-path evidence.
...
`parameter(...)` constructor. Do not guess Palette ids, destinations, fields,
or member paths.
...
StateTree currently advertises no `invoke` operation.
