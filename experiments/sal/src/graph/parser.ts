...
export function parseSalObject(text: string): ObjectResult {
  try {
    const lines = preprocessLines(text);
    if (lines.length === 0) {
      return errorResult("empty_document", "SAL document is empty.");
    }
...
function tryParsePaletteResult(lines: ParsedLine[], context: ParseContext): SalObject | undefined {
