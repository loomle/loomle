...
`UTimelineTemplate`, but the public SAL identity remains its owning Timeline
...
compile. SAL preserves `BS_Unknown`, `BS_Dirty`, `BS_Error`, `BS_UpToDate`,
...
`CLASS_Deprecated` state back to the corresponding Blueprint fields; SAL
...
explicit imports. Canonical SAL sorts its strings for deterministic text; that
...
Blueprint reads return concrete objects directly in the ordered SAL document.
...
and other durable `FBPVariableDescription` state. SAL does not translate those
...
not identity. `asset`, `id`, and `name` are SAL common structure;
...
ownership and flags. SAL does not add a second role enum. Consequently an
...
Those Pins remain the returned state and identity. SAL does not add a Function
...
`GT_Function`. SAL does not create an Interface object, an interface-specific
...
multicast delegate. SAL does not add a `graph` field: UE associates the
...
`UBlueprint::Timelines`. SAL presents that backing pair as one complex Node:
...
Tracks remain nested native Node state rather than SAL objects. Exact reads may
...
all four arrays and are the exact selectors used by Timeline Operations. SAL
...
SAL identity. A Key is selected by Track name, native channel when present, and
...
Template duplication. They are not stable SAL identity and are not writable.
...
Sibling order is derived from statement order for the same parent. SAL text
...
separate design only when a concrete workflow needs it. SAL does not introduce
...
SAL does not maintain parallel `get(...)`, `set(...)`, `event(...)`, or
`call(...)` constructors.
...
keeps that object text and adds comments covering every usable SAL common and UE
...
parallel SAL statement. Direct Macro Library compilation is unavailable because
...
cannot be mapped to an authored SAL object remains an unreferenced original
...
ordinary Blueprint field effects; SAL does not add a Namespace operation.
...
UE declares it as `TSubclassOf<UObject>` with `NoResetToDefault`, so SAL does
...
warnings or errors. SAL preserves that behavior: the mutation remains applied,
...
field because its `Graphs` are UE-created owned structure. SAL does not add an
...
Interface Class Path and matches `FBPInterfaceDescription::Interface`; SAL does
...
SAL therefore completes this validation before calling the native path and
...
SAL list. SAL does not infer the path from `EGraphType`: Function, Interface,
...
unique suffix. SAL permits neither implicit behavior: preflight returns a name
...
Component, Timeline, or override Graph, SAL returns a conflict until the caller
...
confirmation path and may leave invalid call Nodes. SAL does not reproduce an
...
`remove` is the deletion authorization; SAL adds no `force` argument. Preflight
...
child Blueprint Variable, Component, Timeline, or Function Graph. SAL treats
...
existing same-named Graph out of the way; SAL preflight returns a conflict
...
that will become invalid. SAL neither leaves those errors silently nor expands
...
SAL does not add a cross-domain Patch form merely to combine Dispatcher
...
colliding child-Blueprint declaration; SAL treats either implicit repair as a
preflight conflict.
...
path. If the explicit parent is invalid, SAL rejects the operation instead of
...
SAL exposes no SCS Component class conversion. UE's
...
are returned as mutation effects. SAL introduces no writable `Root` field and
...
the remaining children below it before validating the Scene Root set. SAL does
...
UE removes variable getter and setter Nodes for the deleted Component, and SAL
reports those native deletions. UE does not remove Component Bound Event Nodes
that become invalid. If any such usage is resolvable, SAL preflight returns a
...
`Copy`, and `Paste` remain editor clipboard commands rather than SAL Patch
operations.
...
constructor is still `node(...)`; SAL does not add a Timeline constructor:
...
their exact selector. `FKeyHandle` remains transient and never crosses the SAL
boundary:
