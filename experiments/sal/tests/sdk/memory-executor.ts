...
assert.match(collection.text ?? "", /branch\.execute = pin/);
console.log("[PASS] generic memory executor filters collections and returns owned fields");

const exact = await sal.query(`${locator}\nquery g\nnode@N2`);
assert.deepEqual(exact.diagnostics, []);
assert.match(exact.text ?? "", /branch = node/);
console.log("[PASS] generic memory executor resolves typed stable IDs");
