...
global CLI
  ...
  project_install copies LoomleBridge into Project/Plugins/LoomleBridge
...
- Native `project_install` 遇到 Fab/Engine-managed plugin：跳过 plugin copy，
  返回清晰的 skipped 状态；native MCP 仍可通过 `project_attach` 使用它。
- 只有 native/project-local plugin 才由 native `project_install` 和 update sync
  维护。
...
- Mac/Windows release workflow 负责平台验证：Rust CLI、LoomleBridge
  `BuildPlugin`、UE smoke/regression，以及 Python MCP 单测都必须在对应平台
  通过。
- `Finalize LOOMLE Native Release` 只负责 native/官网安装链路：等待 Mac 和
  Windows native assets，合并 native installer manifest，更新 `loomle-latest`。
- Fab workflow 在 native finalize 成功之后运行，生成一个平台无关、source-only
  的 `loomle-fab-plugin.zip`。
- Fab 包内置的是 MCP tool manifest，也就是 `mcp/manifest/manifest.json` 复制到
  `Resources/MCP/tool-manifest/manifest.json`；它不是
  `loomle-manifest.json`，也不应该进入 native installer manifest。
- Fab 包不得包含 `Binaries/`、`Intermediate/`、`Saved/` 或平台二进制文件。
