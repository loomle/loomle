...
The feature was implemented for issue #166 as first-version asset
create/inspect/edit support.
...
1. `userDefinedStruct` is declared in Rust asset schemas and transformed by
   both Rust and Python clients.
2. Internal dispatch routes `blueprint.struct.inspect` and
   `blueprint.struct.edit`.
3. `FLoomleBlueprintAdapter` provides UDS load, serialize, create, inspect,
   edit, type parsing, field resolution, revision, and validation helpers.
4. Schema tests and Python transform tests cover public dispatch.
5. Bridge smoke coverage creates a struct, inspects it, adds a field, and
   verifies final field state.
6. Future negative tests should cover duplicate names, unsupported recursive
   struct type, invalid default value, final-field removal, and revision
   conflict.
