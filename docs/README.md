...
- `0.4.0` CLI contraction for the first cut: `loomle` becomes a pure stdio MCP
  proxy, while install/update/doctor move to scripts.
...
22. `issues/PCG_GRAPH_QUERY_FULL_COVERAGE_UPGRADE.md`
- Local parent issue for making PCG `graph.query` the primary full-coverage node readback surface.

23. `issues/BLUEPRINT_EMBEDDED_TEMPLATE_NODES.md`
...
24. `issues/GRAPH_QUERY_SURFACE_MODEL.md`
...
25. `issues/BLUEPRINT_GAP_PRODUCT_CAPABILITIES.md`
...
26. `issues/JOBS_LONG_RUNNING_TASK_RUNTIME.md`
...
27. `issues/PROFILING_RUNTIME_ANALYSIS_INTERFACE.md`
