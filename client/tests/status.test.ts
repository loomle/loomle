...
const githubRelease = {
  ...
  prerelease: false,
  assets: [{
    name: "loomle-bridge-0.7.1.zip",
    state: "uploaded",
    browser_download_url: "https://github.com/loomle/loomle/releases/download/v0.7.1/loomle-bridge-0.7.1.zip",
...
test("offers the latest stable GitHub Release to prerelease and stable Clients", async () => {
  ...
  assert.deepEqual(await releases.check("0.7.1-rc.1", "darwin-arm64"), {
    status: "available",
    version: "0.7.1",
    releaseUrl: "https://github.com/loomle/loomle/releases/tag/v0.7.1",
    assetUrl: "https://github.com/loomle/loomle/releases/download/v0.7.1/loomle-bridge-0.7.1.zip",
    sha256: "a".repeat(64),
  });
  ...
  assert.deepEqual(await releases.check("0.7.1", "win32-x64"), {
    status: "current",
  });
  assert.deepEqual(await releases.check("0.8.0-rc.1", "darwin-arm64"), {
    status: "current",
  });
});
...
test("keeps malformed, offline, and unsupported update discovery informational", async () => {
  ...
  assert.deepEqual(await checker(githubRelease).check("0.7.0-rc.1", "linux-x64"), {
    status: "unknown",
...
test("requests the public latest Release endpoint with explicit GitHub headers", async () => {
  ...
  assert.deepEqual(await releases.check("0.7.1", "darwin-arm64"), { status: "current" });
...
test("requires one uploaded versioned asset with GitHub's SHA-256 digest", async () => {
  for (const release of [
    ...
    {
      ...githubRelease,
      assets: [{ ...githubRelease.assets[0], digest: "sha256:invalid" }],
    },
    {
      ...githubRelease,
      assets: [{ ...githubRelease.assets[0], browser_download_url: "https://example.test/file.zip" }],
    },
  ]) {
    assert.deepEqual(await checker(release).check("0.7.0", "darwin-arm64"), {
      status: "unknown",
      reason: "invalid_github_release",
    });
  }
