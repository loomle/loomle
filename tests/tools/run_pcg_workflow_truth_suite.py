...
def verify_graph(client: McpStdioClient, request_id: int, asset_path: str) -> dict[str, Any]:
    ...
    if payload.get("status") == "error":
        raise WorkflowSuiteError("verify_gap", f"workflow pcg_compile returned error: {compact_json(payload)}")
    compile_report = payload.get("compileReport")
    if not isinstance(compile_report, dict) or compile_report.get("compiled") is not True:
        raise WorkflowSuiteError("verify_gap", f"workflow pcg_compile missing compiled=true: {compact_json(payload)}")
    diagnostics = payload.get("diagnostics")
    if not isinstance(diagnostics, list):
        raise WorkflowSuiteError("verify_gap", f"workflow pcg_compile missing diagnostics[]: {compact_json(payload)}")
    return {
...
def run_workflow_case(
    ...
    *,
    ...
    try:
        ...
        mutate_result = call_tool(client, request_id_base + 10, "pcg_graph_edit", mutate_args)
