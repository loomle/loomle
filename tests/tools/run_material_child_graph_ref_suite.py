...
def _query_graph_by_ref(client: McpStdioClient, request_id: int, *, graph_ref: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(snapshot, dict):
        raise MaterialChildGraphRefSuiteError("child_graph_ref_unqueryable", f"material_graph_inspect(graph) missing semanticSnapshot: {compact_json(payload)}")
    return snapshot
...
def _query_graph_by_name(client: McpStdioClient, request_id: int, *, asset_path: str, graph_name: str) -> dict[str, Any]:
    ...
    snapshot = payload.get("semanticSnapshot")
    if not isinstance(snapshot, dict):
        raise MaterialChildGraphRefSuiteError("child_graph_ref_unqueryable", f"material_graph_inspect(graphName) missing semanticSnapshot: {compact_json(payload)}")
    return snapshot
    ...
    *,
    ...
    try:
        ...
        if not isinstance(child_graph_ref, dict):
            raise MaterialChildGraphRefSuiteError("child_graph_ref_unsurfaced", f"material_graph_inspect missing childGraphRef: {compact_json(node)}")
