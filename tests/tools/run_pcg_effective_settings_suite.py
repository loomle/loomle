...
def _query_effective_settings(node: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(effective_settings, dict):
        raise EffectiveSettingsSuiteError(
            ...
            f"pcg.graph.inspect missing effectiveSettings: {compact_json(node)}",
        )
    ...
    *,
    ...
    try:
        ...
        if not isinstance(node, dict):
            raise EffectiveSettingsSuiteError("query_structure_gap", f"pcg.graph.inspect missing node {node_id}")
