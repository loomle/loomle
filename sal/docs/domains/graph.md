...
`pin@id` resolution is scoped to the already bound Graph. A PinId used by a Pin
in another Graph is unrelated and must not make this selector ambiguous. Inside
the bound Graph, the adapter resolves the selector only when exactly one Pin
matches. Multiple matching Pins report `resolution.pin_ambiguous`; the adapter
must not select the first Pin in authored Node order. Loomle does not add a
second composite Pin-reference syntax for malformed native state.
...
Pin identity restoration follows UE ownership. `PinId` matching and duplicate
validation are local to one owning Node; the same PinId on another Node or in
another Graph does not make the source Blueprint invalid. A duplicate PinId
inside one Node remains ambiguous and fails closed when the sandbox cannot map
the source and copied Pins one-to-one. Such a local defect must not become a
Blueprint-wide uniqueness rule that blocks unrelated Blueprint or Widget
mutations.
